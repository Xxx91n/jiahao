...
## T-2 dispositions (R2 implementation round, 2026-09-18)

Executed under the R1 stage gate (spr on grill-t16-docs; boundary green on the self-check battery). Each row names its evidence; this section is the same-commit ledger note for the R2 machinery edits.

- defer-0004 — stays deferred; the registry row is untouched this round (the narrowed re-defer stands). The verifier fix is machinery-side: check-ci-jobs.js exit is keyed to defer0004.satisfied per ADR-0077 D-A (settles F-A), the header registers the convention verbatim + discharge re-point clause (settles F-B), a crash exits 2 via stderr (never masquerades as unsatisfied), and defer0026 legs stay computed/printed as diagnostics. adr-0058-wiring pins the three exit directions, the crash fixture, and the defer0026 diagnostic.
- mechanism_output_diff — checker support landed in check-governance-inventory.js (closed-enumeration membership + R2 classification + bare-marker hard-fail; never feeds burn-rate). Probe-row fixtures live in adr-0076-wiring.
- Facts canon — scripts/build-round-facts.js regenerates .scratch/grill-t16/round-facts.json at the closing step (suites/passed/skipped/pack_bytes/instrument_entries/rewrite_map_citations/registry_entries/anchors_count/battery_as_of_commit/report_commit:null/not_run[]); the report facts section renders deterministically inside a sentinel region; the artifact stays out of the anchors chain (disclosed deviation).
- F-E nits executed: surface-taxonomy.js dead m[1]||m[2] dropped; check-ci-jobs.js any_matrix regex now matches the inline matrix: {...} form (fixture pinned); t15 ledger citation count corrected 1346 -> 1403 (both copies resynced, anchors regenerated). The adr-0069 stale comment rode along with the forced 76->77 count bump in R1 (same physical line; disclosed, not hidden).
- defer-0058 (standing per-round net-increment review) — EXECUTED for grill-t16 R2: no further registry net increment in the implementation phase (defer-0062 was the +1, registered and closed in R1).
- Consent-sweep verification: the T-1 disposition lines stand - defer-0060 stays pending-evaluation (quarterly, 2026-12-15), the sunset counter stays 1/6 (next observation 2026-12-15), defer-0053/0057 stay pending-evaluation, O-E backlog stays deferred; zero owner asks this round; the three audit patches (tq/nl/xu) stay untracked forever.
- decision-ledger-t16.md — governance copy admitted to the anchors chain under ADR-0077 (15 -> 16 artifacts); resynced at round close.
