...
- Mechanization note (2026-09-26, grill-t28 D-005/D-006): pinned-sha ancestry assertion mechanized - violation-instance of the existing claim-point contract; the non-ff trigger maps to the existing post-seal-edit red state, no new normative state introduced. Carrier: gates.json `orphan-ancestry` leg (order 219) + fixture tests + surface-taxonomy `freshness.orphan_ancestry` registration + AGENTS.md post-restack ritual; the lane-ownership precheck is rejected (GitButler lane/ownership data is not in the git database - not mechanically portable) and waiver is rejected for mechanically detectable orphaning (waivers cover undetectable residuals only). The alpha classification (violation-instance, zero new normative state) is logged in the t28 ledger pending owner countersign - never self-certified. Promotion hook: if a second independent violation form of the ancestry contract appears, or the assertion's semantics diverge from the claim-point walk, mint a first-class ADR at that point.
