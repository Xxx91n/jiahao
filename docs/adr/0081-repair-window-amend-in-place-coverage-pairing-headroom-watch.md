...
- The t22 trend row declares `carve_out_used:1` naming the round's three R2 touches (`.github/workflows/ci.yml` - the ADR-0057 D-C suite-parity bump 75->76 forced by the new adr-0081 wiring suite; `scripts/check-governance-inventory.js` - the keyed unresolvable-ref repair under audit-r2 R2-C-3; `bench/polygraph/thresholds.json` - the `_doc_*` field trims from the R2-C-1 pack-surface compression) plus the faithful g6-publish replay regeneration on the mechanism-output channel. The compression's registry-row and CONTEXT-prose portion is an R3 documentation-surface edit riding no machinery channel; the thresholds.json trim is the R2 component registered above. (Enumeration corrected in place 2026-09-22 under grill-t23 disposition T3-C-1: this bullet previously read "two R2 touches" and scoped the entire compression pass as R3.)
