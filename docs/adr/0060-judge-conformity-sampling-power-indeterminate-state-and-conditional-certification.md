...
- `npm test`, `npm run gate:all`, `npm run corpus:drift`, `npm pack --dry-run`
  ...
  budget-status note). **Currently breached: the tarball measures 206,501 bytes
  (92 files, re-measured after each packed-file edit); the resolution is an open
  ADR decision, so this Acceptance item is NOT met.** `git diff --check`.
