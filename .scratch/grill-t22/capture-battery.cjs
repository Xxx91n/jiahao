// grill-t22 closeout battery capture (Non-Interpolating Channel, ledger
// D-006 lineage via t20/t21): the grill-t21 battery re-run verbatim
// parameterized --round grill-t22. Every leg runs through a spawnSync arg
// array - never an escape-interpreting string layer - and each evidence
// file is written via fs in the "$ <command>\n\nEXIT <n>\n\n<output>" shape.
// t22 deltas:
//  - expected-suites 76 (the adr-0081 wiring suite lands this round)
//  - adr-0081-wiring.txt leg captures the new suite verbatim
//  - coverage.txt leg re-anchors at the t22 round base b93a5df (ADR-0081
//    D-A: the base pairs with the latest row's own round)
//  - coverage-badref.txt leg: audit-r2 R2-C-3 repair proof - an
//    unresolvable ref exits 1 with a keyed FAIL, no uncaught stack
//  - quoted-stale fixture carries the t22 canon values
//  - NEVER_COMMIT widened: audit2-evidence/ (this audit's never-commit
//    class) joins audit\d*-evidence/ under one regex
...
// verbatim argv helpers: the '$' line IS the spawned argv. 'node' names
// process.execPath verbatim - the spawned binary is node itself.
...
const fw = node('scripts/build-round-facts.js', ['--round', 'grill-t22']);
console.log('[facts write] exit ' + fw.status); if (fw.status !== 0) { console.log(fw.out); process.exit(1); }
const sp = node('scripts/build-round-facts.js', ['--round', 'grill-t22', '--report', REPORT]);
...
cap('check-ci-jobs-missing.txt', shownNode(['scripts/check-ci-jobs.js', '.scratch/grill-t22/evidence/no-such-ci.yml']), node('scripts/check-ci-jobs.js', ['.scratch/grill-t22/evidence/no-such-ci.yml']));
...
cap('quoted-stale.txt', shownNode(['scripts/build-round-facts.js', '--round', 'grill-t22', '--check', '--report', '.scratch/grill-t22/evidence/quoted-stale.fixture.md']), node('scripts/build-round-facts.js', ['--round', 'grill-t22', '--check', '--report', '.scratch/grill-t22/evidence/quoted-stale.fixture.md']));
