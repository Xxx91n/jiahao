// grill-t20 AUDIT battery re-capture (second-party, never-commit evidence).
// Mirrors .scratch/grill-t20/capture-battery.cjs verbatim --round grill-t20,
...
// Added audit legs: adr-0079-wiring, adr-0076-wiring, round-facts-missing.
...
const EVD = path.join(ROOT, '.scratch', 'grill-t20', 'audit-evidence');
...
// AUDIT: mutating legs (facts write + report splice) intentionally
// omitted - they would rewrite committed canon. The --check legs below
// verify committed round-facts.json + report region against regeneration.
...
cap('quoted-stale.txt', shownNode(['scripts/build-round-facts.js', '--round', 'grill-t20', '--check', '--report', '.scratch/grill-t20/audit-evidence/quoted-stale.fixture.md']), node('scripts/build-round-facts.js', ['--round', 'grill-t20', '--check', '--report', '.scratch/grill-t20/audit-evidence/quoted-stale.fixture.md']));
...
// audit-added legs: focused new-surface wiring + missing-facts boundary
...
const MFIX = '.scratch/grill-t20/audit-evidence/missing-facts.fixture.md';
fs.writeFileSync(path.join(ROOT, MFIX), '# fixture report\n', 'utf8');
cap('round-facts-missing.txt', shownNode(['scripts/build-round-facts.js', '--round', 'grill-t20-audit-nofacts', '--check', '--report', MFIX]), node('scripts/build-round-facts.js', ['--round', 'grill-t20-audit-nofacts', '--check', '--report', MFIX]));
...
const SELF = '.scratch/grill-t20/audit-evidence/clean-tree.txt';
