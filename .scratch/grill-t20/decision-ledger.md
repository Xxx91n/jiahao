...
- t21 revision (2026-09-22, ledger D-002): the t20 row now records `carve_out_used:1` - the carve-out channel is used as designed rather than annotated as a non-use; the `README-zh-CN.md` listing is the taxonomy-residual misclassification carried on the reused `gtd.files` channel (CONTEXT Trend Anchor disclosed-repair clause). The four claim sites carry the revised markers; ADR-0080 lands the taxonomy amendment + the owed t20-ledger anchors admission.
