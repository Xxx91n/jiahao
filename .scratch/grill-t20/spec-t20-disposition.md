...
In scope: B-1..B-7 dispositions (R3 in-round / defer-register / Disclosed Repair per section-2); the inherited README charter (EN-primary redesign, zh-CN mirror, ADR-0079, trend row); setup commit absorbing pz + kv (t19 audit report + audit handoff, untracked at t19 audit-final); closeout per section-7. Out of scope: any R2 machinery edit (build-round-facts.js stays untouched — B-4/B-5 go to the registry, not the code); any README-side change to the install.js pointer contract ('Verifier deployment discipline' name stands); stack landing; never-commit set (audit-evidence trees incl. grill-t19, round patches, round-commits).
