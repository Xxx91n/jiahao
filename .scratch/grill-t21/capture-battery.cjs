// grill-t21 closeout battery capture (Non-Interpolating Channel, ledger
// D-006 lineage via t20): the grill-t20 battery re-run verbatim
// parameterized --round grill-t21. Every leg runs through a spawnSync arg
// array - never an escape-interpreting string layer - and each evidence
// file is written via fs in the "$ <command>\n\nEXIT <n>\n\n<output>" shape.
// t21 deltas (audit C-6 smells are registered defer-0066, not fixed here):
...
const EVD = path.join(ROOT, '.scratch', 'grill-t21', 'evidence');
...
const fw = node('scripts/build-round-facts.js', ['--round', 'grill-t21']);
console.log('[facts write] exit ' + fw.status); if (fw.status !== 0) { console.log(fw.out); process.exit(1); }
const sp = node('scripts/build-round-facts.js', ['--round', 'grill-t21', '--report', REPORT]);
...
const SELF = '.scratch/grill-t21/evidence/clean-tree.txt';
