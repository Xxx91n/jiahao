// grill-t21 SECOND audit battery re-capture (verifier-profile second-party
// agent, grill-t22 T-0 audit window; never-commit evidence).
// Mechanical transform of the COMMITTED .scratch/grill-t21/capture-battery.cjs
// (post-repair form): mutating legs (facts write + report splice) omitted -
// the --check legs verify the committed canon instead. Output -> audit2-evidence/.
// Added audit legs over the audit1 precedent harness:
//  - coverage.txt re-run (verbatim committed leg, --coverage-base dc6d21b...)
//  - coverage-wrong-base.txt (must-fail: base 2ecbddb pulls in
//    scripts/build-round-facts.js, R2, undeclared on the latest row)
//  - coverage-badref.txt (probe: unresolvable ref behaviour)
//  - coverage-fixture.txt (module-level must-fail: the committed diff vs a
//    row whose gtd.files omits bench/polygraph/thresholds.json)
//  - reclass-from-pin.txt (jest -t 'from:R2' - the rc.from tightening pin)
//  - claim-sites.txt (node claim-sites.cjs: grep-able repair-claim checks)
// clean-tree NEVER_COMMIT regex extended to classify this audit2-evidence/
// tree (disclosed delta; the committed regex names only audit-evidence/).
'use strict';
const fs = require('fs');
...
const EVD = path.join(ROOT, '.scratch', 'grill-t21', 'audit2-evidence');
...
cap('quoted-stale.txt', shownNode(['scripts/build-round-facts.js', '--round', 'grill-t21', '--check', '--report', '.scratch/grill-t21/audit2-evidence/quoted-stale.fixture.md']), node('scripts/build-round-facts.js', ['--round', 'grill-t21', '--check', '--report', '.scratch/grill-t21/audit2-evidence/quoted-stale.fixture.md']));
...
// audit-added legs (audit1 precedent) + repair-window legs
const MFIX = '.scratch/grill-t21/audit2-evidence/missing-facts.fixture.md';
...
// repair-window legs: the coverage leg must have TEETH. Wrong base pulls an
// R2 file (scripts/build-round-facts.js, touched pre-t21) into the diff that
// the latest row never declared -> exit 1 expected.
cap('coverage-wrong-base.txt', shownNode(['scripts/check-governance-inventory.js', '--coverage-base', '2ecbddb']), node('scripts/check-governance-inventory.js', ['--coverage-base', '2ecbddb']));
...
// module-level must-fail fixture: committed diff vs a row omitting thresholds.json
cap('coverage-fixture.txt', shownNode([path.join('.scratch', 'grill-t21', 'audit2-evidence', 'coverage-fixture.cjs')]), node(path.join('.scratch', 'grill-t21', 'audit2-evidence', 'coverage-fixture.cjs'), []));
// the rc.from tightening pin (source-text pin + positive control, per the suite)
cap('reclass-from-pin.txt', shownNode([JEST, 'test/adr-0080-wiring.test.js', '-t', 'from:R2']), jest(['test/adr-0080-wiring.test.js', '-t', 'from:R2']));
// grep-able claim-site checks (C-1..C-5 repair claims on the live tree)
cap('claim-sites.txt', shownNode([path.join('.scratch', 'grill-t21', 'audit2-evidence', 'claim-sites.cjs')]), node(path.join('.scratch', 'grill-t21', 'audit2-evidence', 'claim-sites.cjs'), []));
...
// class or this leg's own outputs (self-disclosed). AUDIT DELTA: the
// never-commit regex is extended to classify this audit2-evidence/ tree.
const SELF = '.scratch/grill-t21/audit2-evidence/clean-tree.txt';
const NEVER_COMMIT = /^\.scratch\/[^/]+\/audit2?-evidence\/|\.patch$|round-commits\.txt$/;
...
console.log('audit2 battery capture complete');
